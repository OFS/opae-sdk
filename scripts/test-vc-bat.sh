  ...
  case $1 in 
       *"bmc"*)
           ...
           if [[ $2 =~ ${OUTPUT_MACRO[0]} && $2 =~ ${OUTPUT_MACRO[1]} && 
                 ...
                 $2 =~ ${OUTPUT_MACRO[18]} && $2 =~ ${OUTPUT_MACRO[19]} ]]
           then
               parse_ec=0
           else
               parse_ec=1 
       ...
       *"power"*)
           ...
           if [[ $2 =~ ${OUTPUT_MACRO[0]} && $2 =~ ${OUTPUT_MACRO[1]} && 
                 ...
                 $2 =~ ${OUTPUT_MACRO[13]} && $2 =~ ${OUTPUT_MACRO[14]} ]]
           then
               parse_ec=0
           else
               parse_ec=1
       ...
       *"phy"*)
           echo "fpgainfo phy"
           if [[ $2 =~ "Port0" && $2 =~ "Port1" && $2 =~ "Port2" && $2 =~ "Port3" && 
                 $2 =~ "Port4" && $2 =~ "Port5" && $2 =~ "Port6" && $2 =~ "Port7" ]]
           then
               parse_ec=0
           else
               parse_ec=1
...
"nlb0 -id 9AEFFE5F-8457-0612-C000-C9660D824272"
...
do
  ...
  parse_ec=0
  if [[ ${tool_test} == "fpgainfo"* && ${output} == *"0x0B3"* ]]
  then
  ...
  then
    ...
    failed_tests="$failed_tests $tool_test"
  fi
done
...
echo "These test(s) Failed:$failed_tests"
