...
class SignCmfApproveCommand(SignCommand):
    '''
    Approve a sign request for a CMF file / zip of CMF files
    '''

    def __init__(self):
        super(SignCmfApproveCommand,self).__init__()
        self.name = 'sign_cmf_approve'

    def add_parser(self, parser):
        super(SignCmfApproveCommand,self).add_parser(parser=parser)
        ...
        self.parser.add_argument('--approval-role', metavar='<css_approval_role>')
    ...
    def execute(self, args):
        super(SignCmfApproveCommand, self).execute(args=args)

        self.execute_cmd(sign_keypair_file=args.signkeyfile, keychainfile=args.keychainfile, cmf_file=args.infile, \
                         approval_role=args.approval_role, sign_request_id=args.sign_request_id, id_map_file=args.id_map_file, \
    ...
    def execute_cmd(self, sign_keypair_file, keychainfile, cmf_file, approval_role, sign_request_id, id_map_file, css_project, css_keyhash, signtool):
        self.css = Css()
        super(SignCmfApproveCommand, self).execute_cmd(signtool=signtool)

        cmf = self.load_cmf(cmf_file)
        self.initialize_sign_params(keychainfile, sign_keypair_file, css_project, css_keyhash)

        self.css.sign_cmf_approve(cmf, approval_role=approval_role, sign_request_id=sign_request_id, id_map_file=id_map_file)
