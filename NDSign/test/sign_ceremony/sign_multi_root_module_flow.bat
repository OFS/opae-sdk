@REM This script will walk you though the basic steps to sign a modules for multi root formatted keychain
...
@REM ---------------------------------------------------------------------------
@echo Step 1: Create multi root formatted root.qky file (Master of Sign Ceremony)
@REM call ndsign create_root_key --help

call %NDSIGN_INTERACTIVE% create_root_key --multi=1 --keyfile=%NDSIGN_ROOT%\keys\css\PSGTest2_Release_pubkey.bin PSGTest2_root.qky
...
call %NDSIGN_INTERACTIVE% create_pubkey_module --multi=1 %NDSIGN_ROOT%\keys\css\PSGTest1_Debug_pubkey.bin %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% sign_module_request --multi=1 --permissions=0x1 --cancel_id=0x0 --css-project=PSGTest2 %NDSIGN_ROOT%\keys\css\PSGTest2.qky %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% sign_module_approve --multi=1 --approval_type reviewer %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% --debug sign_module_approve --multi=1 --approval_type %REVIEWER2% %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% --debug sign_module_approve --multi=1 --approval_type %REVIEWER3% %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% sign_module_approve --multi=1 --approval_type manager %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% sign_module_approve --multi=1 --approval_type security %UNSIGNED_MODULE_FILE%
...
call %NDSIGN_INTERACTIVE% sign_module_finalize --multi=1 PSGTest2_root.qky %UNSIGNED_MODULE_FILE% PSGTest1_codesign1_signed.qky
@REM ---------------------------------------------------------------------------
