...
TEST_F(Reconf_f0, PR_drv_01)
{
	...
	// Reconfigure gbs
	EXPECT_EQ(FPGA_NOT_SUPPORTED, fpgaReconfigureSlot(h, slot,bitstream,bitstream_len,0));
...
TEST_F(Reconf_f0, PR_drv_02)
{
	...
	// NULL Driver hnadle
	EXPECT_EQ(FPGA_NOT_SUPPORTED, fpgaReconfigureSlot(NULL, slot,bitstream,bitstream_len,0));
	...
	EXPECT_EQ(FPGA_NOT_SUPPORTED, fpgaReconfigureSlot(h, slot,NULL,bitstream_len,0));

	// invalid bitstream size
	EXPECT_EQ(FPGA_NOT_SUPPORTED, fpgaReconfigureSlot(h, slot,bitstream,0,0));

	// invalid bitstream slot
	EXPECT_EQ(FPGA_NOT_SUPPORTED, fpgaReconfigureSlot(h, 44,bitstream,bitstream_len,0));
