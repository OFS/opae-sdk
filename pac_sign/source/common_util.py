...
def assert_in_error(boolean, string, *arg):
    ...
    if boolean:
        ...
        info = "Module: %s, Function: %s, Line: %d" % (module, caller[3],
                                                       caller[2])
        msg = "%s\n       %s" % (info, string)
        ...
        caller = inspect.stack()[1]
        module = os.path.basename(caller[1])
        msg = "Module: %s, Function: %s, Line: %d" % (module, caller[3],
                                                      caller[2])
        print_error(msg, space=0)
        sys.excepthook = exception_handler
        assert False, msg
...
def check_extension(file, extension):
    ...
    if (file_char_count == 0 or extension_char_count == 0 or file_char_count <
        (extension_char_count + 1)
...
def get_password(messages, MIN, MAX, comment=""):
    ...
    if len(messages) == 2:
        echo = [
            "python",
            "-c",
            ("import socket; import getpass; HOST, PORT = '127.0.0.1', %d; " +
             "sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM); " +
             "sock.connect((HOST, PORT)); %sa=getpass.getpass('%s'); " +
             "b=getpass.getpass('%s'); sock.send(a.encode('utf-8')); " +
             "sock.recv(2048); sock.send(b.encode('utf-8')); sock.close()") %
            (randomport, comment, messages[0], messages[1]),
        ]
    else:
        echo = [
            "python",
            ...
            ("import socket; import getpass; HOST, PORT = '127.0.0.1', %d; " +
             "sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM); " +
             "sock.connect((HOST, PORT)); %sa=getpass.getpass('%s'); " +
             "sock.send(a.encode('utf-8')); sock.close()") %
            (randomport, comment, messages[0]),
        ]
    ...
    if len(password1) >= MIN and len(password1) <= MAX:
        ...
        if len(messages) == 2:
            if password2 is not None and len(password1) == len(password2):
...
class BYTE_ARRAY:
    ...
    def get_dword(self, offset):
        ...
        for i in range(4):
            assert (offset < self.size()
                    ), "Data size is %d, attemp to access index %d" % (
                        self.size(), offset)
