...
class _KEY(object):
    ...
    def match_xy(self, public_key):
        ...
        cpointer = self.get_char_point_from_bignum(x, self.curve_info.size)
        cpointer.compare_data(self.xy.data[:self.curve_info.size],
...
class _PRIVATE_KEY(_KEY):
    ...
    def sign(self, sha, data, fixed_RS_size=0):
        ...
        # assign r
        cpointer = self.get_char_point_from_bignum(signature.contents.r,
        ...
        # assign s
        cpointer = self.get_char_point_from_bignum(signature.contents.s,
                                                   self.curve_info.size)
        data.append_data(cpointer.data)
        del cpointer
